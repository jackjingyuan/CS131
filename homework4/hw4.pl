...
trans([1,1], .).
trans([1,1], -).
trans([1,1,1,1|Tail], Result) :- \+(member(0, Tail)), trans([1,1,1], Result).
...
trans([0,0,0,0,0], ^).
trans([0,0,0,0,0], #).
trans([0,0,0,0,0,0,0,0|Tail], Result) :- \+(member(1, Tail)), trans([0,0,0,0,0,0,0], Result).

separation(Pattern, []) :- trans(Pattern, dihdahs).
separation(Pattern, [^]) :- trans(Pattern, ^).
separation(Pattern, [#]) :- trans(Pattern, #).

signal_morse([], []).
signal_morse(B, [M]) :- trans(B, M).
signal_morse(B, [M]) :- append(Word, Pattern, B), trans(Pattern, dihdahs), trans(Word, M).
signal_morse(B, [M]) :- append(Word, Pattern, B), trans(Pattern, ^), trans(Word, M).
signal_morse(B, [M]) :- append(Word, Pattern, B), trans(Pattern, #), trans(Word, M).
signal_morse(B, M) :- 
	append(Head, Tail, B), separation(Pattern, Sign), append(Pattern, [1|PatternTail], Tail), 
	trans(Head, Trans), Trans \= dihdahs, append([Trans], Sign, TransHead), 
